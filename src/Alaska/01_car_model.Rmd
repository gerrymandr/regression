...
prec_shp <- readOGR(dsn = "C:/Users/ckell/Dropbox/Gerrycamp Regressions Project/Alaska Project/data/SW Proc Shape Files", layer = "2013-SW-Proc-Shape-files")
...
precinct_shape$id <- row.names(precinct_shape)
precinct_shape@data <- left_join(precinct_shape@data, demog, by = (GEOID = "GEOID"))
precinct_shape@data <- left_join(precinct_shape@data, vote, by = (GEOID = "GEOID"))
```
...
model <- lm(formula=form, data=precinct_shape@data)
...
W.nb <- poly2nb(precinct_shape, row.names = rownames(precinct_shape@data))
