...
#d_mod_dat$int <- rep(1, nrow(d_mod_dat))
#d_mod_dat <- cbind(d_mod_dat[,1], d_mod_dat[,10], d_mod_dat[,2:9])#, mod_mat)
d_mod_dat <- cbind(d_mod_dat, mod_mat)
#colnames(d_mod_dat)[1:2]<- c("D.Dist.Vote", "int")
#r_mod_dat$int <- rep(1, nrow(r_mod_dat))
#r_mod_dat <- cbind(r_mod_dat[,1], r_mod_dat[,10], r_mod_dat[,2:9])#, mod_dat)
r_mod_dat <- cbind(r_mod_dat, mod_mat)
#colnames(r_mod_dat)[1:2]<- c("R.Dist.Vote", "int")
...
test2 <- resid(c.d.glm_model) 

#test2 <- c.d.glm_model$residuals
test2 <- as.data.frame(test2)
test3 <- c.d.glm_model$fitted.values
actual_d <- d_mod_dat$D.Dist.Vote
full_dat <- cbind(actual_d, test3, test2)
full_dat$resid <- full_dat[,1]-full_dat[,2]
?residuals
...
v_table_lm <- pred_val(coeffs.Dem, coeffs.Rep, test)
...
v_table_glm <- pred_val(coeffs.Dem, coeffs.Rep, test)

length(which(v_table_glm[,2] > v_table_glm[,3])) #still only predicts 4
