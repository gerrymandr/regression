...
test <- cbind(d.pred.ler, actual.d)
...
pred_val <- function(coeffs.Dem, coeffs.Rep, mod_dat){
  ...
  #tableDem <- coeffs.Dem[1] + exp(as.matrix(mod_dat[,2:9])%*%coeffs.Dem[2:9])
  tableDem <- as.matrix(mod_dat)%*%coeffs.Dem
  tableDem <- as.numeric(tableDem)
  tableDem <- exp(tableDem)
...
v_table_lm <- pred_val(coeffs.Dem, coeffs.Rep, mod_dat2)
...
v_table_glm <- pred_val(coeffs.Dem, coeffs.Rep, mod_dat2)
