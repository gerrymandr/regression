...
#Mayer Response: BBWSADEM12, BBWSAREP12
#TP (third party)

#approximately 50% of the data is contested (vs uncontested)
#right now, I will just use the uncontested data
length(c(which(ward_shp$BBWSADEM12 == 0 ), which(ward_shp$BBWSAREP12==0)))
unc <- unique(c(which(ward_shp$BBWSADEM12 == 0 ), which(ward_shp$BBWSAREP12 == 0 )))

con_shp <- ward_shp[-unc,]

#non-spatial modeling
d.form <- BBWSADEM12 ~ BBPRESDEM12 + BBPRESREP12+PERSONS18 +BLACK18 + WHITE18 + HISPANIC18+ IncumD_Pop + IncumR_Pop #+ County#try adding fixed effects for county
r.form <- BBWSAREP12 ~ BBPRESDEM12 + BBPRESREP12+PERSONS18 +BLACK18 + WHITE18 + HISPANIC18+ IncumD_Pop + IncumR_Pop #+ County#try adding fixed effects for county
#d.model <- lm(formula=d.form, data=ward_shp@data)
#d.resid.model <- residuals(d.model)
#r.model <- lm(formula=r.form, data=ward_shp@data)
#r.resid.model <- residuals(r.model)

c.d.model <- lm(formula=d.form, data=con_shp@data)
c.d.resid.model <- con_shp@data$BBWSADEM12 - c.d.model$fitted.values
c.r.model <- lm(formula=r.form, data=con_shp@data)
c.r.resid.model <- con_shp@data$BBWSAREP12 - c.r.model$fitted.values



#glm model
#d.glm_model <- glm(formula=d.form, family = "poisson", data=ward_shp@data)
#d.resid.glm <- residuals(d.glm_model)
#r.glm_model <- glm(formula=r.form, family = "poisson", data=ward_shp@data)
#r.resid.glm <- residuals(r.glm_model)

c.d.glm_model <- glm(formula=d.form, family = "poisson", data=con_shp@data)
c.d.resid.glm <- con_shp@data$BBWSADEM12 - c.d.glm_model$fitted.values
c.r.glm_model <- glm(formula=r.form, family = "poisson", data=con_shp@data)
c.r.resid.glm <- con_shp@data$BBWSAREP12 - c.r.glm_model$fitted.values


#need to remove shape file objects with no BBWSADEM12
#no_dem <- ward_shp[-which(is.na(ward_shp$BBWSADEM12)),]
#no_rep <- ward_shp[-which(is.na(ward_shp$BBWSAREP12)),]
...
#W.nb <- poly2nb(ward_shp, row.names = rownames(ward_shp@data))
...
```{r}
actual.d <- nonb_shp@data$BBWSADEM12
actual.r <- nonb_shp@data$BBWSAREP12
...
actual_tab <- cbind(con_shp@data$District, con_shp@data$BBWSADEM12, con_shp@data$BBWSAREP12)
actual_tab <- as.data.frame(actual_tab)
colnames(actual_tab) <- c("District", "actual_d", "actual_r")
...
#First, I will create the model dataframe
mod_dat <- demog_vote[,c("BBPRESDEM12", "BBPRESREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]
d_mod_dat <- con_shp@data[,c("BBWSADEM12", "BBPRESDEM12", "BBPRESREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]
r_mod_dat <- con_shp@data[,c("BBWSAREP12", "BBPRESDEM12", "BBPRESREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]
