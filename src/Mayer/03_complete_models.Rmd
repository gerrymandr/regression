...
#d.form <- CWSADEM12 ~ CPREDEM12 + CPREREP12+PERSONS18 +BLACK18 + WHITE18 + HISPANIC18+ IncumD_Pop + IncumR_Pop 
#r.form <- CWSAREP12 ~ CPREDEM12 + CPREREP12+PERSONS18 +BLACK18 + WHITE18 + HISPANIC18+ IncumD_Pop + IncumR_Pop 
...
Check for overdispersion and implement 
```{r}
...
glm.fit.quasi <- glm(formula=d.form, family = "quasipoisson", data=con_shp@data, control = list(maxit = 1000))
summary(glm.fit.quasi)

### Model Diagnostics
fitted.mean <- (predict.glm(glm.fit, type='link'))
fitted.resp <- (predict.glm(glm.fit, type='response'))
fitted.var <- (con_shp@data$CWSADEM12 - fitted.mean)^2
...
mse <- sum(fitted.var)/nrow(full_data)
...
con_shp@data$ASM.x <- as.numeric(con_shp@data$ASM.x)
actual_tab <- cbind(con_shp@data$ASM.x, con_shp@data$CWSADEM12, con_shp@data$CWSAREP12)
...
mod_dat <- demog_vote[,c("CPREDEM12", "CPREREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]
mod_dat$int <- rep(1, nrow(mod_dat))
mod_dat <- cbind(mod_dat[,9], mod_dat[,1:8])

mod_dat2 <- con_shp@data[,c("CPREDEM12", "CPREREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]
...
#First, I will create the model dataframe
mod_dat <- demog_vote[,c("CPREDEM12", "CPREREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]
d_mod_dat <- con_shp@data[,c("CWSADEM12", "CPREDEM12", "CPREREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]
r_mod_dat <- con_shp@data[,c("CWSAREP12", "CPREDEM12", "CPREREP12", "PERSONS18", "BLACK18", "WHITE18", "HISPANIC18", "IncumD_Pop", "IncumR_Pop")]

con_shp@data$CON <- as.numeric(as.character(con_shp@data$CON))
length(unique(con_shp@data$CNTY_FIPS))

mod_mat <- model.matrix( ~ CNTY_NAME.x - 1, data=con_shp@data )
d_mod_dat <- cbind(d_mod_dat, mod_mat)
r_mod_dat <- cbind(r_mod_dat, mod_mat)
mod_mat2 <- model.matrix( ~ CNTY_NAME - 1, data=demog_vote)
mod_dat$int <- rep(1, nrow(mod_dat))
...
#save.image(file = "C:/Users/ckell/Desktop/Summer 2018/MGGG/Regression/data/wi.final_workspace.Rdata")
