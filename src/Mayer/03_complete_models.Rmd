...
con_actual_tab <- cbind(ForR$assembly, ForR$c_wsa_dem12, ForR$c_wsa_rep12)
...
length(which(v_table_lm[,2] > v_table_lm[,3])) #predicts 41
...
length(which(v_table_glm[,2] > v_table_glm[,3])) #still only predicts 37
...
```{r}
#I believe there is an error in District 53
...
co_dat <- mod_dat[,-80] #also taking out countyWood 
...
length(which(co_v_table_lm[,2] > co_v_table_lm[,3])) #predicts 44
...
length(which(co_v_table_glm[,2] > co_v_table_glm[,3])) #predicts 43
